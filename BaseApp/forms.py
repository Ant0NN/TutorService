...
CHOICES = ((1, 'Кто вы?'), (2, 'Репетитор'), (3, 'Ученик'))
...
class Registration(forms.Form):
    ...
    address = forms.CharField(widget=forms.TextInput(attrs={'placeholder': 'Адрес'}), label='',required=False)
    category = forms.CharField(widget=forms.Select(choices=CHOICES, attrs={'ng-model': 'checked'}), label='', required=True)
...
class Log_in(forms.Form):
    username = forms.CharField(max_length=50, min_length=4, widget=forms.TextInput(attrs={'placeholder': 'Логин'}))
...
class Tutor_settings_form(forms.ModelForm):
    experience = forms.IntegerField(label='', widget=forms.TextInput(attrs={'placeholder': 'Стаж', 'type': 'text'}), required=True)
...
class Additional_inf_settings_form(forms.ModelForm):
    ...
    price = forms.IntegerField(widget=forms.TextInput(attrs={'placeholder': 'Цена', 'type': 'text'}), label='', required=True)
    class Meta:
...
class Additions_form(forms.Form):
    subject_name = forms.CharField(widget=forms.TextInput(attrs={'placeholder': 'Предмет', 'type': 'text'}), label='', required=True)
    section = forms.CharField(widget=forms.TextInput(attrs={'placeholder': 'Раздел', 'type': 'text'}), label='', required=True)
    additions = forms.CharField(widget=forms.TextInput(attrs={'placeholder': 'Дополнение', 'type': 'text'}), label='', required=False)
    pupil_category = forms.CharField(widget=forms.TextInput(attrs={'placeholder': 'Категория ученика', 'type': 'text'}), label='', required=True)
    price = forms.IntegerField(widget=forms.TextInput(attrs={'placeholder': 'Цена', 'type': 'text'}), label='', required=True)

CHOICES_for_contact_form = (('студент', 'Cтудент'), ('школьник', 'Школьник'), ('работающий', 'Работающий'))
...
class Contact_form(forms.Form):
    ...
    category = forms.CharField(widget=forms.Select(choices=CHOICES_for_contact_form), label='', required=True)
